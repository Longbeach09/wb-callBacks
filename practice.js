...
const last = (array, callback) => callback(array[1]);
...
const contains = (array, name, callback) => callback(array.includes(name));
...
const evens = (array) => array.filter((number) => number % 2 === 0);
...
const startWithLetterA = (array) =>
  array.filter((stringWithA) => stringWithA.toLowerCase().startsWith("a"));
...
const formalGreeting = (array) => array.map((element) => "Hello, " + element);
...
const sortNumbers = (array) => array.sort((num1, num2) => num1 - num2);
...
const findProgrammer = (array) =>
  array.filter((employee) => employee.job === "programmer")[0];
...
let orderTotals = orders.map((element) => (element.tax + 1) * element.price);
...
const productOfArray = (array) =>
  array.reduce((runningTotal, curValue) => runningTotal * curValue, 1);
